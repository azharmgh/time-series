...
def getData(targeturl, startdate):
    ...
    if (records > 0):
            ...
            df_up.to_csv(filepath,mode='a',header=False)
...
def refreshData():
    ...
    print('today date is ' + str(endDate))
    lastdate = getLastDateFromFile()
    last = datetime.strptime(str(lastdate),'%Y-%m-%d %H:%M:%S')
    print('last date is ' + str(last))
    delta = endDate - last
...
def getLastDateFromFile():

    df = pd.read_csv(filepath)
    lastdate = df.tail(1)['TimeStamp'].tolist()
    retdate = datetime.strptime(str(lastdate[0]),'%Y-%m-%d %H:%M:%S')
    print(retdate)
    return retdate
